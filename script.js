...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (registerForm) { // Check if the registerForm exists
        registerForm.addEventListener('submit', function(e) {
            var submitButton = document.getElementById('btn'); // Change the id based on your actual button id
            if (submitButton.disabled) {
                e.preventDefault(); // Prevent multiple form submissions
            } else {
                submitButton.disabled = true; // Disable button on first click
            }
        });
    }
});
			...
			for (var i = 0; i < data.length; ++i) {
				switch (data[i]) {
					...
					case 9:
						...
						break;
					case 10:
						document.getElementById('errs').innerHTML += '<div class="err">This organization already has a registered user. You cannot register with this email.</div>';
						break;
					case 11:
						document.getElementById('errs').innerHTML += '<div class="err">No organization with this contact email found. Please contact your administrator.</div>';
						break;
					default:
						document.getElementById('errs').innerHTML += '<div class="err">An unknown error occurred. Please try again later.</div>';
				}
