<?php 
   ...
   function loadAwaitingApprovalTasks($orgId) {
      $C = connect();
      ...
      // SQL query to join work_sessions, tasks, employees, and break_sessions
      $query = "
               SELECT 
                  ws.id AS work_session_id,
                  ws.start_time AS session_start_time,
                  ws.end_time AS session_end_time,
                  ws.approved AS session_approved,
                  t.id AS task_id,
                  ...
                  GROUP_CONCAT(bs.id) AS break_session_ids,
                  GROUP_CONCAT(bs.break_start) AS break_start_times,
                  GROUP_CONCAT(bs.break_end) AS break_end_times,
                  SUM(bs.duration) AS total_break_duration
               FROM 
                  work_sessions ws
               JOIN 
                  tasks t ON ws.task_id = t.id
               JOIN 
                  employees e ON ws.employee_id = e.id
               LEFT JOIN 
                  break_sessions bs ON bs.work_session_id = ws.id
               WHERE 
                  ws.organization_id = ? AND ws.approved = 0
               GROUP BY 
                  ws.id
      ";
  
      $res = sqlSelect($C, $query, 'i', $orgId);
  
      $tasksAwaitingApproval = [];
      if ($res) {
          while ($row = $res->fetch_assoc()) {
              $tasksAwaitingApproval[] = $row;
          }
      }
  
      return $tasksAwaitingApproval;
  }  
   ...
   function loadOrgDetails($orgId) {
      ...
      FROM organizations 
      WHERE organizations.id=?', 'i', $orgId);
   ...
   $employees = loadEmployeesByOrg(orgId: $organizationId);
   $tasks = loadTasksByOrg($organizationId);
   $tasksAwaitingApproval = loadAwaitingApprovalTasks($organizationId);
?>
...
<aside id="separator-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 py-4 overflow-y-auto bg-gray-800">
      <a href="?page=dashboard" class="flex items-center ps-2.5 mb-5">
            <span class="self-center text-xl font-semibold whitespace-nowrap text-white"><?php echo $_SESSION['org_label_name']; ?></span>
      </a>
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="?page=waitingAproval" class="flex items-center p-2 text-gray-900 rounded-lg text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
               <span class="flex-1 ms-3 whitespace-nowrap">Vagter til godkendelse</span>
            </a>
         </li>
      </ul>
      <ul class="pt-4 mt-4 space-y-2 font-medium border-t border-gray-700">
         ...
         <li>
            <a href="?page=employees" class="flex items-center p-2 text-gray-900 rounded-lg text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
               <span class="flex-1 ms-3 whitespace-nowrap">Kunder</span>
            </a>
         </li>
      </ul>
      <ul class="pt-4 mt-4 space-y-2 font-medium border-t border-gray-700">
...
<div class="sm:ml-64 dark:bg-gray-900">
    <?php
        switch($page) {
            ...
            case 'waitingAproval':
               $groupedTasks = [];
               foreach ($tasksAwaitingApproval as $task) {
                   $employeeId = $task['employee_id'];
               
                   if (!isset($groupedTasks[$employeeId])) {
                       $groupedTasks[$employeeId] = [
                           'employee_details' => [
                               'id' => $employeeId,
                               'firstname' => $task['employee_firstname'],
                               'lastname' => $task['employee_lastname'],
                   ...
                   $groupedTasks[$employeeId]['tasks'][] = $task;
               }
               ...
               echo '            
                  ...
                  foreach ($groupedTasks as $employeeId => $employeeData) {
                     echo '
                        <div class=" overflow-x-auto pb-4">
                              <div class="block">
                                 <p class="px-3 py-3 text-left whitespace-nowrap text-xl font-semibold text-gray-900 dark:text-white capitalize">'. htmlspecialchars($employeeData['employee_details']['firstname'] . ' ' . $employeeData['employee_details']['lastname']).'</p>
                                 <p class="px-3 pb-4 text-left whitespace-nowrap text-base font-semibold text-gray-900 dark:text-white capitalize">Uge 40</p>
                                 <div class="overflow-x-auto w-full border rounded-lg dark:border-transparent">
                                    <table id="employees-table" class="w-full rounded-xl">
                                          <thead>
                                             <tr class="bg-gray-50 dark:bg-gray-800">
                                                ...
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white min-w-[150px]"> Dato </th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white capitalize"> Projekt </th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white min-w-[150px]"> Kommentar </th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white capitalize"> Tid </th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white capitalize"> Pause </th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 dark:text-white capitalize"> Rediger </th>
                                             </tr>
                                          </thead>
                                          <tbody class="divide-y divide-gray-300 dark:divide-gray-600">';
                                          usort($employeeData['tasks'], function($a, $b) {
                                             // Convert session_end_time to timestamps for comparison
                                             return strtotime($b['session_end_time'] ?? '1970-01-01') - strtotime($a['session_end_time'] ?? '1970-01-01');
                                         });
                                         
                                         
                                          // Foreach loop to populate the table rows
                                          foreach ($employeeData['tasks'] as $awaitingTask) {
                                             echo '<tr class="bg-white dark:bg-gray-700 transition-all duration-500 hover:bg-gray-50 dark:hover:bg-gray-600">';
                                             ...
                                             <th class="">
                                                <div class="flex items-center py-5 px-5 ">
                                                   <input type="checkbox" data-task-id="'.htmlspecialchars($awaitingTask['work_session_id']).'" data-employee-id="'.htmlspecialchars($employeeData['employee_details']['id']).'" value="" class="w-5 h-5 appearance-none border border-gray-300  rounded-md mr-2 hover:border-indigo-500 hover:bg-indigo-100 checked:bg-no-repeat checked:bg-center checked:border-indigo-500 checked:bg-indigo-100">
                                                </div>
                                             </th>  
                                             ';
                                             ...
                                             <td class=" px-5 py-3">
                                                <div class="w-24 flex items-center gap-3">
                                                   <div class="data">
                                                      <p class="font-normal text-sm text-gray-900 dark:text-slate-200">'.htmlspecialchars(date('d-m-Y', strtotime($awaitingTask["session_start_time"]))).'</p>
                                                      <p class="font-normal text-xs leading-5 text-gray-400 dark:text-slate-400">'.htmlspecialchars(date('H:i', strtotime($awaitingTask["session_start_time"]))).' - ' .htmlspecialchars(date('H:i', strtotime($awaitingTask["session_end_time"]))).'</p>
                                                   </div>
                                                </div>
                                             </td>';
                                             
                                             echo '<td class="p-5 max-w-xs truncate text-sm leading-6 font-medium text-gray-900 dark:text-slate-200">'.htmlspecialchars($awaitingTask['task_title']). '</td>';
                                             

                                             // Beskrivelse
                                             echo '<td class="p-5 max-w-xs truncate text-sm leading-6 font-medium text-gray-900 dark:text-slate-200">' . htmlspecialchars($awaitingTask['task_description']) . '</td>';

                                             // Tid
                                             $start_time_parts = explode(':', date('H:i', strtotime($awaitingTask["session_start_time"])));
                                             $end_time_parts = explode(':', date('H:i', strtotime($awaitingTask["session_end_time"])));
                                             
                                             $start_seconds = ($start_time_parts[0] * 3600) + ($start_time_parts[1] * 60);
                                             $end_seconds = ($end_time_parts[0] * 3600) + ($end_time_parts[1] * 60);
                                             
                                             // Calculate the difference, considering if the end time is on the next day
                                             if ($end_seconds < $start_seconds) {
                                                 // Add 24 hours to the end time if it crosses midnight
                                                 $end_seconds += 24 * 3600;
                                             }
                                             
                                             $duration_in_seconds = $end_seconds - $start_seconds;
                                             ...
                                             echo '<td class="p-5 max-w-xs truncate text-sm leading-6 font-medium text-gray-900 dark:text-slate-200">' .
                                                 htmlspecialchars(sprintf('%02dt, %02dm', $hours, $minutes)) .
                                                 '</td>';
                                             ...
                                             $totalDurationSeconds = 0; // Variable to accumulate the total break duration in seconds
                                             
                                             if (!empty($awaitingTask['break_session_ids'])) {
                                                 // Parse the concatenated break session data
                                                 $breakSessionIds = explode(',', $awaitingTask['break_session_ids']);
                                                 $breakStartTimes = explode(',', $awaitingTask['break_start_times']);
                                                 $breakEndTimes = explode(',', $awaitingTask['break_end_times']);
                                             
                                                 foreach ($breakSessionIds as $index => $breakId) {
                                                     // Calculate break duration in seconds
                                                     $startTime = strtotime($breakStartTimes[$index]);
                                                     $endTime = strtotime($breakEndTimes[$index]);
                                                     $durationSeconds = $endTime - $startTime;
                                                     
                                                     // Accumulate the break duration
                                                     $totalDurationSeconds += $durationSeconds;
                                                 }
                                             
                                                 // Convert total break duration to minutes and seconds
                                                 $totalMinutes = floor($totalDurationSeconds / 60);
                                                 $totalSeconds = $totalDurationSeconds % 60;
                                             
                                                 // Display the total break duration in minutes and seconds
                                                 echo '
                                                 <div class="w-24 flex items-center gap-3">
                                                     <div class="data">
                                                         <p class="font-normal text-sm text-gray-900 dark:text-slate-200">' . htmlspecialchars($totalMinutes) . 'm, ' . htmlspecialchars($totalSeconds) . 's</p>
                                                     </div>
                                                 </div>';
                                             } else {
                                                 // Handle cases where there are no breaks
                                                 echo '<p class="font-normal text-sm text-gray-400 dark:text-slate-400">Ingen pauser</p>';
                                             }
                                             ...
                                             echo '
                                                <td class="flex p-5 items-center gap-0.5">
                                                      <button id="editWorkSession" class="p-2  rounded-full bg-white dark:bg-slate-600 group transition-all duration-500 hover:bg-indigo-600 flex item-center" data-id="' . htmlspecialchars($awaitingTask['work_session_id']) .'">
                                                         <svg class="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path class="fill-indigo-500 group-hover:fill-white" d="M9.53414 8.15675L8.96459 7.59496L8.96459 7.59496L9.53414 8.15675ZM13.8911 3.73968L13.3215 3.17789V3.17789L13.8911 3.73968ZM16.3154 3.75892L15.7367 4.31126L15.7367 4.31127L16.3154 3.75892ZM16.38 3.82658L16.9587 3.27423L16.9587 3.27423L16.38 3.82658ZM16.3401 6.13595L15.7803 5.56438L16.3401 6.13595ZM11.9186 10.4658L12.4784 11.0374L11.9186 10.4658ZM11.1223 10.9017L10.9404 10.1226V10.1226L11.1223 10.9017ZM9.07259 10.9951L8.52556 11.5788L8.52556 11.5788L9.07259 10.9951ZM9.09713 8.9664L9.87963 9.1328V9.1328L9.09713 8.9664ZM9.05721 10.9803L8.49542 11.5498H8.49542L9.05721 10.9803ZM17.1679 4.99458L16.368 4.98075V4.98075L17.1679 4.99458ZM15.1107 2.8693L15.1171 2.06932L15.1107 2.8693ZM9.22851 8.51246L8.52589 8.12992L8.52452 8.13247L9.22851 8.51246ZM9.22567 8.51772L8.52168 8.13773L8.5203 8.1403L9.22567 8.51772ZM11.5684 10.7654L11.9531 11.4668L11.9536 11.4666L11.5684 10.7654ZM11.5669 10.7662L11.9507 11.4681L11.9516 11.4676L11.5669 10.7662ZM11.3235 3.30005C11.7654 3.30005 12.1
8.1403C8.39951 8.36605 8.35444 8.61274 8.31462 8.8L9.87963 9.1328ZM8.52452 8.13247L8.52168 8.13773L9.92967 8.89772L9.9325 8.89246L8.52452 8.13247ZM11.3589 9.89426C11.27 9.98132 11.2252 10.0248 11.1909 10.055C11.1635 10.0791 11.1658 10.0738 11.1832 10.0642L11.9536 11.4666C12.1727 11.3462 12.3427 11.1703 12.4784 11.0374L11.3589 9.89426ZM11.3042 11.6807C11.4912 11.6371 11.7319 11.5878 11.9507 11.4681L11.1831 10.0643C11.2007 10.0547 11.206 10.0557 11.1697 10.0663C11.1248 10.0793 11.0628 10.0941 10.9404 10.1226L11.3042 11.6807ZM11.1837 10.064L11.1822 10.0648L11.9516 11.4676L11.9531 11.4668L11.1837 10.064ZM16.399 6.10097L13.8984 3.60094L12.7672 4.73243L15.2677 7.23246L16.399 6.10097ZM10.8333 16.7001H9.16667V18.3001H10.8333V16.7001ZM3.3 10.8334V9.16672H1.7V10.8334H3.3ZM9.16667 3.30005H11.3235V1.70005H9.16667V3.30005ZM16.7 9.55887V10.8334H18.3V9.55887H16.7ZM9.16667 16.7001C7.5727 16.7001 6.45771 16.6984 5.61569 16.5851C4.79669 16.475 4.35674 16.2728 4.042 15.9581L2.91063 17.0894C3.5722 17.751 4.40607 18.0369 5.4025 18.1709C6.37591 18.3018 7.61793 18.3001 9.16667 18.3001V16.7001ZM1.7 10.8334C1.7 12.3821 1.6983 13.6241 1.82917 14.5976C1.96314 15.594 2.24905 16.4279 2.91063 17.0894L4.042 15.9581C3.72726 15.6433 3.52502 15.2034 3.41491 14.3844C3.3017 13.5423 3.3 12.4273 3.3 10.8334H1.7ZM10.8333 18.3001C12.3821 18.3001 13.6241 18.3018 14.5975 18.1709C15.5939 18.0369 16.4278 17.751 17.0894 17.0894L15.958 15.9581C15.6433 16.2728 15.2033 16.475 14.3843 16.5851C13.5423 16.6984 12.4273 16.7001 10.8333 16.7001V18.3001ZM16.7 10.8334C16.7 12.4274 16.6983 13.5423 16.5851 14.3844C16.475 15.2034 16.2727 15.6433 15.958 15.9581L17.0894 17.0894C17.7509 16.4279 18.0369 15.594 18.1708 14.5976C18.3017 13.6241 18.3 12.3821 18.3 10.8334H16.7ZM3.3 9.16672C3.3 7.57275 3.3017 6.45776 3.41491 5.61574C3.52502 4.79674 3.72726 4.35679 4.042 4.04205L2.91063 2.91068C2.24905 3.57225 1.96314 4.40612 1.82917 5.40255C1.6983 6.37596 1.7 7.61798 1.7 9.16672H3.3ZM9.16667 1.70005C7.61793 1.70005 6.37591 1.69835 5.4025 1.82922C4.40607 1.96319 3.5722 2.24911 2.91063 2.91068L4.042 4.04205C4.35674 3.72731 4.79669 3.52507 5.61569 3.41496C6.45771 3.30175 7.5727 3.30005 9.16667 3.30005V1.70005Z" fill="#818CF8"></path>
                                                         </svg>
                                                      </button>
                              ...
                              <div class="mx-2 my-4">
                                 <button type="button" onclick="approveTasks(' . htmlspecialchars($employeeData['employee_details']['id']) .');" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Godkend</button>
                              </div>
                        </div>
                     </div>                       
